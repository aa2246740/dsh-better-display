...
- **执行中**：思考、工具和中途说明保持真实顺序。Skill / Read / Bash、上下文来源、文件路径和原始详情仍然可读。若模型再次开始思考，此前步骤会收进**一个**可展开的盒子，摘要只统计结构（例如 `思考×1 · 输出×1 · 工具×1`），不改写 Think 原文；当前这段思考及其后的输出、工具仍保持展开。中途补充消息会重新开始这一段。单独出现的输出或工具不会触发收起。
...
51 项单元测试覆盖顺序、轮次结束、异常保留、两行跟随、Unicode、Markdown、流式缓冲，以及执行中「再次思考时收起此前步骤」。安装后仍需检查真实宿主行为，不能把单元测试通过当作页面已加载。
